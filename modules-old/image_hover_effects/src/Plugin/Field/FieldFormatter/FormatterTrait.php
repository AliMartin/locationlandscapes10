...
trait FormatterTrait {
  ...
  protected function buildSettingsForm(&$element) {
    ...
    $element['hover_effect'] = [
      '#type' => 'select',
      '#title' => $this->t('Image Hover Effect'),
      '#options' => ['' => $this->t('- None -')] + self::getEffects(),
      '#default_value' => $settings['hover_effect'],
      '#states' => $states,
    ];
    $element['hover_text'] = [
      '#type' => 'textarea',
      '#title' => $this->t('Hover text'),
      '#default_value' => $settings['hover_text'],
      '#rows' => 3,
      '#attributes' => ['style' => 'max-width: 180px;'],
      '#states' => $states,
      '#description' => $this->t('You may use tokens to display entity data. Example: [node:title]'),
    ];
  ...
  protected function buildSettingsSummary() {
    ...
    if ($image_link) {
      if ($settings['hover_effect']) {
        ...
        $summary[] = $this->t('Hover effect: %effect', ['%effect' => $effects[$settings['hover_effect']]]);
      }
      $summary[] = $this->t('Hover text: %text', ['%text' => $settings['hover_text']]);
    }
    return $summary;
  }
